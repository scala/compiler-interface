...
public interface IncrementalCompiler {
   ...
   *
   * @param inputs An instance of {@link Inputs} that collect all the inputs required to run the
   *     compiler (from sources and classpath, to compilation order, previous results, current
   *     setup, etc).
   * @param logger An instance of {@link Logger} that logs Zinc output.
   * @return An instance of {@link CompileResult} that holds information about the results of the
   *     compilation.
   */
   ...
   * @param perClasspathEntryLookup Lookup of data structures and operations for a given classpath
   *     entry.
   * @param reporter An instance of {@link Reporter} to report compiler output.
   * @param compileOrder The order in which Java and Scala sources should be compiled.
   * @param skip Flag to ignore this compilation run and return previous one.
   * @param progress An instance of {@link CompileProgress} to keep track of the current compilation
   *     progress.
   * @param incrementalOptions An Instance of {@link IncOptions} that configures the incremental
   *     compiler behaviour.
   * @param temporaryClassesDirectory A directory where incremental compiler can put temporary class
   *     files or jars.
