...
public interface VirtualFileRef {
  ...
  /**
   * Returns unique identifier for the file. For Java compilation it needs to contain the path
   * structure matching the package name.
   *
   * <p>However, it must end with the same value as name(), Java files must end with ".java", and
   * Scala files must end with ".scala".
   */
  public String id();

  /**
   * Returns "file name" for the file. Java files must end with ".java", and Scala files must end
   * with ".scala".
   */
  public String name();
